...
export const StackedColumnChart: React.FC<StackedColumnChartProps> = ({
  ...
  // Convert StackedChartConfig to ColumnChartConfig
  ...
  if (!stackedResult) {
    return (
      <div className={`stacked-column-chart ${className}`} style={{ width, height }}>
  ...
  return (
    <div className={`stacked-column-chart ${className}`}>
      {/* Legend */}
      {seriesData.length > 1 && (
      ...
      {/* Totals display */}
      {config.showTotals && config.totalPosition === 'top' && (
      ...
      <ColumnChart
        ...
        className="stacked-column-chart-content"
      />
    </div>
  )
}
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { StackedColumnChart, default: StackedColumnChart }
}